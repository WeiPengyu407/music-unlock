...
离线根本解不开。本软件自研 ekey 链：**自动从本机 QQ 音乐客户端（或浏览器）提取登录态**，
...
| 酷狗（旧版） | .kgm/.kgma | um 引擎，本地秒解 | 不需要 |
| 酷狗（新版） | .kgg | um `--kgg-db`，密钥在酷狗 PC 客户端本地数据里 | 一次酷狗 PC 登录 |
| 酷我 / 虾米等 | .kwm/.xm 等 | um 引擎，本地秒解 | 不需要 |
| 咪咕 | .mg3d | 纯本地减法密码 | 不需要 |
...
1. 拖入文件，程序检测到新格式会先在本机静默查找酷狗 PC 客户端数据；找到即不用管
2. 找不到才弹窗，点【导入酷狗登录态】——Windows 会找 `%APPDATA%\KuGou8\KGMusicV3.db`
3. 仍没有则提示打开酷狗音乐 PC 版并登录，然后点【我已登录，重新导入】；
   实在找不到再手动选客户端目录里的 `KGMusicV3.db`
4. 客户端数据路径会记住，之后同格式不用再登录

### QQ 音乐新版（.mflac / .mgg）

1. 拖入文件，程序检测到新格式会弹窗提示需要 QQ 登录态
2. 点【导入QQ登录态】——Windows 先读 QQ 音乐客户端，再读 Edge / Chrome / QQ浏览器 / 360
3. 都没有则提示先打开 QQ 音乐并登录，然后点【我已登录，重新导入】
