...
| 酷狗（新版） | .kgg | um `--kgg-db`，密钥在酷狗 PC 客户端的 KGMusicV3.db | 导入一次密钥库 |
...
密钥不在文件里，而在酷狗 PC 客户端（win32 v11）的 `KGMusicV3.db`。
程序会先在常见位置（本机 Wine/CrossOver、Windows 的 AppData）自动找库；
找不到就点【导入酷狗密钥库】选一次，之后记住路径。
